...
contract MLDSA_ExpandA_KAT_Test is Test {
    ...
    function test_expandA_synth_matches_kat() public {
        ...
        string memory json =
            vm.readFile("test_vectors/expandA_synth_001.json");
        ...
        bytes memory row0Col0Bytes =
            vm.parseJson(json, ".row0_col0_first8");
        uint256[] memory row0Col0 =
            abi.decode(row0Col0Bytes, (uint256[]));

        bytes memory row1Col2Bytes =
            vm.parseJson(json, ".row1_col2_first8");
        uint256[] memory row1Col2 =
            abi.decode(row1Col2Bytes, (uint256[]));
        ...
        MLDSA65_PolyVec.PolyVecK[5] memory A =
            MLDSA65_ExpandA_Synthetic_FIPSShape.expandA(rho);

        // 5) Перевіряємо перші 8 коефіцієнтів для [row=0, col=0]
        for (uint256 i = 0; i < 8; ++i) {
            uint256 got = uint256(
                uint32(uint256(int256(A[0].polys[0][i])))
            );
            assertEq(
                got,
                row0Col0[i],
        ...
        for (uint256 i = 0; i < 8; ++i) {
            uint256 got = uint256(
                uint32(uint256(int256(A[1].polys[2][i])))
            );
            assertEq(
                got,
                row1Col2[i],
