...
contract NTT_MLDSA_Structure_Test is Test {
    ...
    /// -----------------------------------------------------------------------
    /// sanity-test: NTT та INTT не падають і тримають значення в межах поля
    /// -----------------------------------------------------------------------
    ...
    function test_NTT_RoundtripRandom() public {
        ...
        unchecked {
            for (uint256 i = 0; i < N; i++) {
                a[i] = uint256(keccak256(abi.encodePacked(i, "ntt_random"))) % Q;
            }
        }
    ...
    function test_NTT_RoundtripBasisVectors() public {
        ...
        for (uint256 k = 0; k < 8; k++) {
            ...
            back = NTT_MLDSA_Core.intt(NTT_MLDSA_Core.ntt(e));
